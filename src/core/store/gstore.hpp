...
protected:
    ...
    typedef tbb::concurrent_hash_map<sid_t, std::vector<sid_t>> tbb_hash_map;
    typedef tbb::concurrent_hash_map<ikey_t, std::vector<triple_t>, ikey_Hasher> tbb_triple_hash_map;
    typedef tbb::concurrent_hash_map<ikey_t, std::vector<triple_attr_t>, ikey_Hasher> tbb_triple_attr_hash_map;
    ...
    tbb::concurrent_unordered_map <int, std::map<segid_t, rdf_seg_meta_t> > shared_rdf_seg_meta_map;
    std::map<segid_t, rdf_seg_meta_t> rdf_seg_meta_map;

    // id of all local normal preds, free after gstore init
    std::vector<sid_t> all_local_preds;
    ...
    virtual void insert_vp(int tid, const std::vector<triple_t> &pso, const std::vector<triple_t> &pos) = 0;
    ...
    typedef std::vector<std::unordered_map<sid_t, size_t>> triple_map_t;
    ...
    void init_seg_metas(const std::vector<std::vector<triple_t>> &triple_pso,
        ...
        for (int i = 0; i <= get_num_preds(); ++i) {
            index_cnt_map.insert(std::make_pair(i, cnt_t()));
            normal_cnt_map.insert(std::make_pair(i, cnt_t()));
            rdf_seg_meta_map.insert(std::make_pair(segid_t(0, i, IN), rdf_seg_meta_t()));
            rdf_seg_meta_map.insert(std::make_pair(segid_t(0, i, OUT), rdf_seg_meta_t()));
        }
        // init index segment
    ...
    void finalize_init() {
        std::vector<sid_t>().swap(all_local_preds);
...
public:
    ...
    virtual void init(std::vector<std::vector<triple_t>> &triple_pso,
