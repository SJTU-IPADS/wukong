...
private:
    ...
    typedef std::vector<nng_socket *> socket_vector;
...
public:
    TCP_Adaptor(int sid, std::string fname, int port_base, int num_servers, int num_threads)
    ...
    std::string ip_of(int dst_sid) { return ipset[dst_sid]; }

    bool send(int dst_sid, int dst_tid, const std::string &str) {
    ...
    std::string recv(int tid) {
        ...
        std::string s((char *)nng_msg_body(msg), nng_msg_len(msg));
        nng_msg_free(msg);
        return s;
    }

    bool tryrecv(int tid, std::string &s) {
        nng_ms *msg = NULL;
        ...
        s = std::string((char *)nng_msg_body(msg), nng_msg_len(msg));
        nng_msg_free(msg);
        return true;
    }
};
