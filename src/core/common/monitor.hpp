...
private:
    ...
    std::map<int, std::vector<uint64_t>> total_latency_map;
...
public:
    ...
    void print_cdf() {
        ...
        std::vector<double> cdf_rates = {0.01};
        ...
        std::map<int, std::vector<uint64_t>> cdf_res;

        // select 25 points from total_latency_map
        for (auto &e : total_latency_map) {
            query_type = e.first;
            std::vector<uint64_t> &lats = e.second;
