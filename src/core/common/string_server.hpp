...
public:
    ...
    bool exist(std::string str) { return bimap.exist(str); }

    std::string id2str(sid_t sid) { return bimap[sid]; }

    sid_t str2id(std::string str) { return bimap[str]; }

    void add(std::string str, sid_t sid) { bimap.insert_kv(str, sid); }
