...
class Planner {
    ...
    std::unordered_set<ssid_t> contain_k2l;  // index which contains k2l
    //bool enable_merge;        // if non endpoint variable > 3, we enable merge

    // for type-centric method
    std::unordered_map<ssid_t, int> var2ptindex;  // find the first appearance for var
    std::unordered_map<int, int> type2sub;  // e.g. (sub -1) type (type -3)
    ...
    void transfer_to_chains(std::vector<SPARQLQuery::Pattern> &p,
                                std::vector<ssid_t> &attr_pattern,
    ...
    CostResult cal_first_pt(ssid_t v1, ssid_t v2, ssid_t d, ssid_t v3, 
        ...
        if (from_o2 && v2 == TYPE_ID){
            ...
            if (stats->global_single2complex.find(vtype) == stats->global_single2complex.end()) {
                ...
                std::unordered_set<ssid_t> type_set = stats->global_single2complex[vtype];
            ...
            if (v2 == PREDICATE_ID && d == IN) {  // v1 is predicate
                ...
                tycountv = from_o2
                        ? stats->global_tystat.fine_type[std::make_pair(v2, v1type)]
                        : stats->global_tystat.fine_type[std::make_pair(v1type, v2)];
    ...
    CostResult cal_rest_pt(ssid_t o1, ssid_t p, ssid_t d, ssid_t o2,
        ...
        for (size_t i = 0; i < row_num; i++) {
                ...
                if (!from_o2 && o2 > 0){
                    ...
                    if (pre_tyid >= 0) {
                        ...
                        if (stats->global_single2complex.find(o2) != stats->global_single2complex.end()) {
                            cost_result.small_prune = true;
                            std::unordered_set<ssid_t> type_set = stats->global_single2complex[o2];
            ...
            if (!from_o2) {
                ...
                tycountv = stats->global_tystat.fine_type[std::make_pair(pre_tyid, p)];
            } else {
                ...
                tycountv = stats->global_tystat.fine_type[std::make_pair(p, pre_tyid)];
            }
    ...
    void plan_enum(SPARQLQuery &r) {
        ...
        while (!median_results.empty()){
            ...
            std::vector<CostResult> level_results;
            ...
            std::sort(level_results.begin(), level_results.end());
    ...
    bool do_patterns(SPARQLQuery &r, std::vector<SPARQLQuery::Pattern> &patterns, bool test) {
        //input : patterns
    ...
    void set_ptypes_pos(std::vector<int> &ptypes_pos, const std::string &dir, int current_order, int raw_order) {
    ...
    void set_direction(SPARQLQuery::PatternGroup &group,
                       ...
                       std::vector<int> &ptypes_pos = empty_ptypes_pos) {
   ...
   public:
    ...
    bool set_plan(SPARQLQuery::PatternGroup &group, std::istream &fmt_stream,
                  std::vector<int> &ptypes_pos = empty_ptypes_pos) {
        if (!fmt_stream.good()) {
