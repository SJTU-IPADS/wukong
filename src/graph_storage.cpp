...
void
graph_storage::init(RdmaResource* _rdma, uint64_t machine_num,
                    uint64_t machine_id)
{
	rdma        = _rdma;
	m_num       = machine_num;
	...
	header_num	= (slot_num / cluster_size)
	              / indirect_ratio * (indirect_ratio - 1);
...
uint64_t
graph_storage::insertKey(local_key key)
{
	uint64_t vertex_ptr;
...
uint64_t
graph_storage::atomic_alloc_edges(uint64_t num_edge)
{
...
void
graph_storage::atomic_batch_insert(vector<edge_triple>& vec_spo,
...
void
graph_storage::print_memory_usage()
{
...
vertex
graph_storage::get_vertex_local(local_key key)
{
	uint64_t bucket_id = key.hash() % header_num;
	while (true) {
...
vertex
graph_storage::get_vertex_remote(int tid, local_key key)
{
...
edge*
graph_storage::get_edges_global(int tid, uint64_t id, int direction,
...
void
graph_storage::insert_vector(tbb_vector_table& table, uint64_t index_id,
                             uint64_t value_id)
{
...
void
graph_storage::init_index_table()
{
...
edge*
graph_storage::get_index_edges_local(int tid, uint64_t index_id,
