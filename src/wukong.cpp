...
void *agent_thread(void *arg)
{
    ...
    if (enable_binding && core_bindings.count(agent->tid) != 0)
        bind_to_core(core_bindings[#include "core/store/dgraph.hpp"
agent->tid]);
    else
...
main(int argc, char *argv[])
{
    ...
    // load global configs
    load_config(std::string(argv[1]), world.size());
    ...
    std::vector<Broadcast_Mem *> bcast_mems;
    ...
    if (Global::generate_statistics) {
        ...
        t0 = timer::get_usec();
        std::string fname = Global::input_folder + "/statfile";  // using default name
