...
private:
    ...
    public:
        ...
        void print_slot(page_manager_agent &pm_agent) {
            std::cout << get_special() << "," << get_length() << "," << get_pos()
                 << "," << get_page_id() << ","
                 << std::string(pm_agent.get_content_pointer(this), get_length()) << ","
                 << pm_agent.get_value(this) << std::endl;
        }
    };
    ...
    public:
         ...
         */
        void insert_value_in_node(const std::string& prefix, const T v,
                                  bitrie* const bt) {
            value_ = v;
            have_value_ = true;
            bt->set_v2k(v, this, -1);
    ...
    private:
        ...
        private:
            struct fast_path {
                ...
                std::string fast_path_string_;
                node* dest_node_;

            public:
                fast_path(const unsigned int hash_val, const std::string &fast_path_string, node* dest_node)
                ...
                inline const std::string& get_string() const { return fast_path_string_; }
                inline node* get_dest_node() const { return dest_node_; }
            };

            std::vector<fast_path> fast_paths_;
        ...
        public:
            ...
            void insert_fast_path(const char* key, size_t key_size, node* node_ptr) {
                fast_path new_fast_path(hash(key, key_size, 1), std::string(key, key_size), node_ptr);
    ...
    public:
        ...
        void print_slot(int i, int j, page_manager_agent &pm_agent) {
            slot *s = get_slot(i, j);
            std::cout << i * cur_associativity_ + j << ":" << s->get_special() << ","
                 << s->get_length() << "," << s->get_pos() << ","
                 << s->get_page_id() << ",";
            std::string str = std::string(pm_agent.get_content_pointer(s),
                                s->get_length());
            std::cout << str;
    ...
    public:
        hash_node(trie_node* p, const std::string& prefix, page_manager* pm,
        ...
        int cuckoo_hash(size_t bucketid, bitrie* bt) {
            ...
            std::map<T, int> searchPoint_wait_2_be_update;
    ...
    public:
        ...
        std::string get_common_prefix() const {
          ...
          return std::string(ret_key_pointer, common_prefix_len);
        }      
     ...
     */
    trie_node* burst(burst_package bp, trie_node* orig_parent, const std::string &orig_prefix) {
        ...
        std::string common_prefix = bp.get_common_prefix();
        ...
        std::string prefix = orig_prefix + common_prefix;
    ...
    public:
         ...
         */
        std::vector<burst_package*> notify_list;
    ...
    private:
        void init_a_new_page_group(group_type init_type, size_t page_group_index) {
            if (init_type == group_type::SPECIAL_GROUP) {
                ...
                std::cout << "initing a undefined group type page group!" << std::endl;
                assert(false);
                exit(0);
                return;
            }
        ...
        void swap(page_manager* pm, group_type gt) {
            ...
            std::cout << "swapping undefined group type!" << std::endl;
            assert(false);
            exit(0);
            return;
        }
    ...
    class search_point {
       ...
       public:
        ...
        std::string get_string(page_manager* pm) const {
            if (target_node_ == nullptr) return std::string();
    ...
    void insert_kv_in_bitrie(node* start_node, const K_unit* key,
            ...
            switch (current_node->get_node_type()) {
                case node_type::TRIE_NODE: {
                    ...
                    if(current_node == nullptr){
                        std::string new_prefix = std::string(prefix_key, prefix_key_size) + std::string(key, ref_pos);
        ...
        current_node->insert_value_in_node(std::string(prefix_key, prefix_key_size) + std::string(key, key_size), v, this);
        return;
    }
     ...
     */
    void apply_the_changed_search_point(std::map<T, int>& search_points) {
...
public:
    bitrie():pm(new page_manager(1, 1)),
                t_root(new hash_node(nullptr, std::string(), pm)) { }
     ...
     */
    std::string operator[](T v) { return v2k[v].get_string(pm); }
     ...
     */
    T operator[](const std::string& key_string) const {
     ...
     * @return false Target element doesn't exist.
     */
    bool exist(const std::string& key_string) const {
        return search_kv_in_bitrie(t_root, key_string.data(), key_string.size())
            .exist();
    }
     ...
     */
    void insert_kv(const std::string& key_string, T v) {
