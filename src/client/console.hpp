...
{
    std::string str = con_adaptor->recv(tid);
    std::stringstream ss;
    ss << str;
...
static void fail_to_parse(Proxy *proxy, int argc, char** argv)
{
    ...
    std::cout << std::endl
         << "Input \'help\' command to get more information." << std::endl;
}
...
static void run_sparql(Proxy * proxy, int argc, char **argv)
{
    ...
    if (sparql_vm.count("-f")) {
        ...
        if (Global::enable_planner) {
            ...
            std::string fmt_fname = sparql_vm["-p"].as<std::string>();
    ...
    if (sparql_vm.count("-b")) {
        std::string fname = sparql_vm["-b"].as<std::string>();
        std::ifstream ifs(fname);
        if (!ifs.good()) {
            logstream(LOG_ERROR) << "Query file not found: " << fname << LOG_endl;
            fail_to_parse(proxy, argc, argv); // invalid cmd
        ...
        while (getline(ifs, sg_cmd)) {
            ...
            if (sg_argc > 2 && (std::string(sg_argv[0]) == "sparql" && std::string(sg_argv[1]) == "-f")) {
                std::cout << "Run the command: " << sg_cmd << std::endl;
