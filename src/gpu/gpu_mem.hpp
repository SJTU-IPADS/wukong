...
public:
    ...
    public:
        sid_t *get_inbuf() const {
            return ((sid_t*)(reversed ? orbuf : irbuf));
        }

        sid_t *get_outbuf() const {
            return ((sid_t*)(reversed ? irbuf : orbuf));
        }
...
retry:
    ...
    // result buffer
    inline void reverse_rbuf() { ASSERT_MSG(false, "GPUMem::reverse_rbuf is removed!"); }

    inline sid_t *res_inbuf() { ASSERT(false); return nullptr; }
    // inline uint64_t res_inbuf_offset() { return (rbuf_reversed ? orbuf_off : irbuf_off); }

    inline sid_t *res_outbuf() { ASSERT(false); return nullptr; }
