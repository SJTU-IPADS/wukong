...
public:
    ...
    bool result_buf_ready(SPARQLQuery& req) {
        if (req.combined) {
            ...
            for (auto const& job : combined.get_jobs()) {
                if (!job.rbuf_info.loaded) return false;
            }
            return true;
        }
    ...
    void load_result_buf(SPARQLQuery& req, GPUChannel& channel) {
        ...
        if (req.combined) {
            ...
            if (combined.staged) {
                ...
                // clear staged table
                std::vector<sid_t>().swap(combined.staged_table);
            } else {
            ...
            for (; it != combined.get_jobs().end(); ++it) {
                if (it->rbuf_info.loaded) continue;
    ...
    bool execute_one_pattern(SPARQLQuery& req, GPUChannel& channel) {
        ...
        if (Global::enable_vattr && req.get_pattern(req.pattern_step).pred_type != static_cast<char>(SID_t)) {
            ASSERT_MSG(false, "GPUEngine doesn't support attribute");
        }

        // triple pattern with KNOWN predicate
        SPARQLQuery::PatternType pattern_type = req.get_pattern_type();
        switch(pattern_type) {
        case SPARQLQuery::PatternType::K2U:
            known_to_unknown(req, channel);
            break;
