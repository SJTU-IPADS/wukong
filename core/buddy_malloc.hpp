...
public:
    ...
    virtual uint64_t alloc(uint64_t size, int64_t tid = -1) = 0;
...
private:
    ...
    pthread_spinlock_t alloc_free_lock;
    ...
    inline bool is_empty(uint64_t level, int64_t t_id);
    ...
    inline void mark_free(header* start, uint64_t level, int64_t t_id);
    inline void mark_used(header* start, uint64_t level);
    uint64_t get_free_buddy(uint64_t buddy_idx, uint64_t level);
    int64_t get_free_idx(uint64_t need_level, uint64_t& free_level, int64_t t_id);
...
public:
    ...
    uint64_t alloc(uint64_t size, int64_t tid = -1);
...
inline bool Buddy_Malloc::is_empty(uint64_t level, int64_t t_id){
    if(t_id < 0)
        return free_list[level]->next_free_idx == ptr_to_idx((char*) free_list[level]);
    else
        return tmp_free_list[level + t_id * (level_up_bound+1)]->next_free_idx == ptr_to_idx((char*) tmp_free_list[level + t_id * (level_up_bound+1)]);
}

inline uint64_t Buddy_Malloc::truncate_level(uint64_t level){
...
inline uint64_t Buddy_Malloc::size_to_level(uint64_t size){
...
char* Buddy_Malloc::sbrk(uint64_t n) {
    char* ret_val;
    if(top_of_heap == NULL)
        return NULL;
...
// methods useful for alloc and free
inline void Buddy_Malloc::mark_used(header* start, uint64_t level){
    start->in_use = 1;
    start->level = level;
...
inline void Buddy_Malloc::mark_free(header* start, uint64_t level , int64_t t_id){
    start->in_use = 0;
    start->level = level;
    header* free_header;
    if(t_id < 0)
        ...
        free_header = tmp_free_list[level + t_id * (level_up_bound+1)];
    ...
    start->next_free_idx = next_idx;
     ...
     //buddy_idx +/- (1 << level)
uint64_t Buddy_Malloc::get_free_buddy(uint64_t idx, uint64_t level){
...
int64_t Buddy_Malloc::get_free_idx(uint64_t need_level, uint64_t& free_level, int64_t t_id){
    int64_t free_idx = -1;
    for(free_level = need_level; free_level <= level_up_bound; free_level++){
        if (is_empty(free_level, t_id) && free_level != level_up_bound)
            continue;
        if(is_empty(free_level, t_id) && free_level == level_up_bound){
            header *new_heap = (header*)sbrk(sbrk_size);
            if (!new_heap)
                return free_idx;
            mark_free(new_heap, free_level, t_id);
        }
        if(t_id < 0)
            free_idx = free_list[free_level]->next_free_idx;
        else 
            free_idx = tmp_free_list[free_level + (level_up_bound+1) * t_id]->next_free_idx;
        return free_idx;
    }
}

void Buddy_Malloc::init(void* start, uint64_t size, uint64_t n){
    //assert(level_low_bound >= 4);
    //assert(level_up_bound <= 24);
    ...
    pthread_spin_init(&alloc_free_lock, 0);
    ...
    for(int i = level_low_bound; i <= level_up_bound; i++){
     ...
     // alloc memory for freelist
    uint64_t free_list_idx = alloc((level_up_bound - level_low_bound + 1) * size_per_header);

    // new the tmp_freelists for every thread and alloc memory for them
    tmp_free_list = new header*[nthread_parallel_load * (level_up_bound + 2)];
    tmp_free_list_idx = alloc ((level_up_bound - level_low_bound + 1) * size_per_header * nthread_parallel_load);
...
uint64_t Buddy_Malloc::alloc(uint64_t size, int64_t t_id = -1){
    //pthread_spin_lock(&debug_lock);
    uint64_t need_level = size_to_level(size);
    uint64_t free_level;
    int64_t free_idx = -1;
    if(t_id < 0)
        pthread_spin_lock(&alloc_free_lock);
    // find the smallest available block
    free_idx = get_free_idx(need_level, free_level, t_id);
    ...
    for(uint64_t i = free_level-1; i >= need_level; i--)
        mark_free((header*)idx_to_ptr(free_idx + (1LL<<i)), i, t_id);

    mark_used((header*)idx_to_ptr(free_idx), need_level);
    if(t_id < 0)
        pthread_spin_unlock(&alloc_free_lock);

    usage_counter[need_level]++;
    return get_value_idx(free_idx);
}

void Buddy_Malloc::free(uint64_t free_idx){
...
void Buddy_Malloc::merge_freelists(){
    for(int i = level_low_bound; i <= level_up_bound; i++){
...
void Buddy_Malloc::print_memory_usage(){
