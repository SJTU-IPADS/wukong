...
*/
	...
	Network_Node *node = new Network_Node(world_rank,global_num_thread,string(argv[2]));//[0-thread_num-1] are used
	char *buffer= (char*) malloc(total_size);
	memset(buffer,0,total_size);
    RdmaResource *rdma=new RdmaResource(world_size,global_num_thread,
				world_rank,buffer,total_size,rdma_slot_per_thread,msg_slot_per_thread,rdma_size);
	rdma->node = node;
	rdma->Servicing();
	rdma->Connect();
