...
struct klist_key{
	...
	uint64_t useless:15;
	uint64_t id:48;
	klist_key():dir(0),useless(0),id(0){
		dir-=1;
		useless-=1;
		id-=1;
	}
	...
	klist_key(uint64_t i,uint64_t d):id(i),dir(d){
	}
...
public:
	...
	edge_v2* readGlobal_predict(int tid,uint64_t id,int direction,int predict,int* size){
		...
		klist_key key=klist_key(id,direction);
		...
		edge_v2* edge_ptr=(edge_v2*)local_buffer;
		*size=0;
		int idx=0;
		for(int i=0;i<v.val.size;i++){
			if(edge_ptr[i].predict < predict){
				idx++;
			} else if(edge_ptr[i].predict == predict){
				*size=*size+1;
			} else {
				break;
			}
	...
	edge_v2* readLocal_predict(int tid,uint64_t id,int direction,int predict,int* size){
		...
		edge_v2* edge_ptr=getEdgeArray(v.val.ptr);
		*size=0;
		for(int i=0;i<v.val.size;i++){
			if(edge_ptr[i].predict < predict){
				ptr++;
	...
	void batch_insert(vector<edge_triple>& vec_spo,
		...
		while(start<vec_spo.size()){
			...
			klist_key key= klist_key(vec_spo[start].s,para_out);
		...
		while(start<vec_ops.size()){
			...
			klist_key key= klist_key(vec_ops[start].o,para_in);
			uint64_t vertex_ptr=insertKey(key);
			klist_val val= klist_val(end-start,curr_edge_ptr);
			vertex_addr[vertex_ptr].val=val;
			for(uint64_t i=start;i<end;i++){
